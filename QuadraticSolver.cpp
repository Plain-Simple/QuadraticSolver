...
void Roots(double parameters [8]) {
  int discriminant;
  discriminant = parameters[1] * parameters[1] - 4 * parameters[0] *
                 parameters[2];
  if(discriminant < 0) {
    parameters[7] = 1;
  } else {
    parameters[5] = ((-1 * parameters[1]) + sqrt(parameters[1] * parameters[1] - 4 *
                     parameters[0] * parameters[2])) / (2 * parameters[0]);
    parameters[6] = ((-1 * parameters[1]) - sqrt(parameters[1] * parameters[1] - 4 *
                     parameters[0] * parameters[2])) / (2 * parameters[0]);
  }
}
void Vertex(double parameters [8]) {
  parameters[3] = (parameters[1] * -1) / (2 * parameters[0]);
  parameters[4] = parameters[0] * parameters[3] * parameters[3] + parameters[1] *
...
void Options(int settings [7]) {
  ...
  cout << "6. Set Table Parameters...................(" << settings[5] << "," <<
       settings[6] << ")\n";
...
void Table(double parameters [8], int settings [7]) {
  ...
  for(int i = settings[5]; i <= settings[6]; i++) {
    cout << "   " << i << "          " << parameters[0] * i * i + parameters[1] * i
         + parameters[2] << endl;
  }
}
void Display(double parameters [8], int settings [7]) {
  ...
  cout << "Equation: " << parameters[0] << "x^2 + " << parameters[1] << "x + " <<
       parameters[2] << " = 0\n";
  if(parameters[7] == 1) {
    ...
    if(parameters[5] == parameters[6]) {
      ...
      cout << "Roots are x = " << parameters[5] << " and x = " << parameters[6] <<
           endl;
    }
  }
  if(settings[3] == 1 && parameters[7] == 0) {
    cout << "Sum of roots = " << parameters[5] + parameters[6] <<
